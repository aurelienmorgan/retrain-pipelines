...
def test_mf_lightgbm_regress_mlserver():
    data = pseudo_random_generate(DatasetType.TABULAR_REGRESSION, num_samples=1_500)
    temp_dir = tempfile.mkdtemp()
    data_file_path = f"{temp_dir}/synthetic_classif_tab_data_continuous.csv"
    ...
    as_env_var(pipeline_hp_grid, "pipeline_hp_grid", env=env)
    ...
    command = [
        ...
        data_file_path,
        "--buckets_param",
        '{"num_feature1": 100, "num_feature2": 50}',
        "--pipeline_hp_grid",
        "{pipeline_hp_grid}",
        "--cv_folds",
        "2",
...
def test_mf_tabnet_classif_torchserve():
    data = pseudo_random_generate(DatasetType.TABULAR_CLASSIFICATION, num_samples=1_500)
    temp_dir = tempfile.mkdtemp()
    data_file_path = f"{temp_dir}/synthetic_classif_tab_data_4classes.csv"
    data.to_csv(data_file_path, index=False)
