...
Revision ID: e9d2edb5d123
Revises: bdff337fbb86
Create Date: 2026-08-06 10:04:59.197867
...
def upgrade() -> None:
    ...
    op.create_table('task_payload_attrs',
        sa.Column('task_id', sa.Integer(), nullable=False),
        sa.Column('sha', sa.String(), nullable=True),
        sa.Column('disk_ref', sa.String(), nullable=True),
        sa.Column('inline_val', sa.JSON(none_as_null=True), nullable=True),
    ...
    ##############################################################
    ## convert "null" string to NULL in tasks.rank column       ##
    ##############################################################
    # Previously, tasks.rank was mapped without none_as_null=True,
    # causing Python None to be stored as the JSON literal "null"
    # instead of SQL NULL. This makes the column truly nullable.
    #
    # Note: PostgreSQL's `json` type has no `=` operator, so we
    # cast to text for the comparison. SQLite stores JSON as text
    # anyway, so the raw comparison works there.
    conn = op.get_bind()
    if conn.dialect.name == "postgresql":
        op.execute(
            sa.text("UPDATE tasks SET rank = NULL WHERE rank::text = 'null'")
        )
    else:
        op.execute(
            sa.text("UPDATE tasks SET rank = NULL WHERE rank = 'null'")
        )
    ##############################################################
...
def downgrade() -> None:
    """Downgrade schema."""
    ##############################################################
    ## revert NULL to "null" string in tasks.rank column        ##
    ##############################################################
    # Reverts the upgrade conversion so that NULL values are
    # restored as the JSON literal "null" for the old ORM mapping.
    op.execute(
        sa.text("UPDATE tasks SET rank = 'null' WHERE rank IS NULL")
    )
    ##############################################################

    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('executions', sa.Column('context_dump', sqlite.JSON(), nullable=True))
    op.drop_column('executions', 'artifacts_store_root')
    op.drop_column('executions', 'metadata_root')
    op.drop_table('task_payload_attrs')
