...
def _in_subprocess() -> bool:
    """Test whether called from a child process.

    (not the main process that called RichLoggingController.activate()).
    """
...
class _OrigStdoutWrapper:
    ...
    def __init__(self, write_fn: Callable[[str], object]):
...
class _NotebookLogRelay:
    """Relay rich.text.Text log messages from child processes.

    Down to the main notebook cell output.
    ...
    def __init__(
        self,
        orig_stdout_write: Callable[[str], object],
    ) -> None:
        self._orig_write = orig_stdout_write
        self._queue: multiprocessing.Queue = multiprocessing.Queue()
        self._stop_evt = threading.Event()
        self._thread = threading.Thread(target=self._run, name="nb-log-relay", daemon=True)
    ...
    def _run(self) -> None:
        file = cast(IO[str], _OrigStdoutWrapper(self._orig_write))
...
def nb_activate(orig_stdout_write) -> None:
    """Record main PID and start the relay.

    So subprocess log messages reach the notebook cell output safely.
    """
...
def nb_deactivate() -> None:
    """Stop the relay before restoring stdout.

    So the final drain still reaches the notebook cell output.
    """
