...
def dag_svg(dag: DAG | None = None, execution_id: int | None = None) -> str:
    ...
    """
    template_dir = os.path.join(os.path.dirname(__file__), "web_console", "utils", "execution")
    env = Environment(loader=FileSystemLoader(template_dir))
    env.globals["get_text_pixel_width"] = get_text_pixel_width
    template = env.get_template("svg_template.html")

    if dag:
        tasktypes_list, taskgroups_list = dag.to_elements_lists(serializable=True)
    elif execution_id:
        ...
        if in_notebook():
            ...
            with ThreadPoolExecutor(max_workers=1) as pool:
                future = pool.submit(asyncio.run, _fetch_execution_elements(execution_id))
            ...
            tasktypes_list, taskgroups_list = loop.run_until_complete(
        ...
        tasktypes_list = [tasktypes.__dict__ for tasktypes in tasktypes_list]
        ...
        taskgroups_list = (
            [taskgroup.__dict__ for taskgroup in taskgroups_list] if taskgroups_list else []
        )
        for taskgroup_dict in taskgroups_list:
            taskgroup_dict["uuid"] = str(taskgroup_dict["uuid"])
    else:
        raise ValueError("either 'dag' or 'execution_id' must be passed as param.")
    ...
    id_prefix = "".join(random.choices(string.ascii_letters + string.digits, k=5))
    # print(f"id_prefix : {id_prefix}")
    rendering_content = template.render(
        id_prefix=id_prefix, nodes=tasktypes_list, taskgroups=taskgroups_list or []
    )

    static_dir = os.path.join(os.path.dirname(__file__), "web_console", "static")
    with open(os.path.join(static_dir, "svg_dag.css"), encoding="utf-8") as f:
        svg_dag_css = f.read()
