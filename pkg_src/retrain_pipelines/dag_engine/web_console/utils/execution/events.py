...
new_exec_subscribers: list[tuple[asyncio.Queue, ClientInfo]] = []
exec_end_subscribers: list[tuple[asyncio.Queue, ClientInfo]] = []

new_task_subscribers: list[tuple[asyncio.Queue, ClientInfo]] = []
task_end_subscribers: list[tuple[asyncio.Queue, ClientInfo]] = []

task_trace_subscribers: list[tuple[asyncio.Queue, ClientInfo]] = []
...
async def augment_new_task(task_ext_dict: dict) -> None:
    """Augment in-place."""
    # if task_ext is the head of a distributed sub-DAG split-line
    if task_ext_dict["is_parallel"]:
        parallel_lines_style = CustomStyleDict(task_ext_dict["ui_css"], labelUnderlay="#4d0066")
        fill_defaults(parallel_lines_style, GroupTypes.PARALLEL_LINES)
        parallel_line_style = CustomStyleDict(task_ext_dict["ui_css"], labelUnderlay="#4d0066")
        fill_defaults(parallel_line_style, GroupTypes.PARALLEL_LINE)
    ...
    task_ext_style = CustomStyleDict(
    ...
    if taskgroup_uuid:
        taskgroups_hierarchy_response = await taskgroups_hierarchy(UUID(taskgroup_uuid))
        if taskgroups_hierarchy_response.status_code == 200:
            taskgroups_hierarchy_list = json.loads(
            ...
            for taskgroup_dict in taskgroups_hierarchy_list:
                ...
                taskgroup_style = CustomStyleDict(taskgroup_dict["ui_css"])
            ...
            task_ext_dict["ui_css"]["labelUnderlay"] = taskgroups_hierarchy_list[0]["ui_css"][
                "background"
            ]
        else:
            logging.getLogger().warn(taskgroups_hierarchy_response.body.decode("utf-8"))
