...
def _ngrok_start(port: int) -> str | None:
    """Start an ngrok tunnel on *port* and return its public URL.

    Parameters
    ----------
    port : int
        the port on which to open the tunnel.

    Returns
    -------
    str
        tunnel_url, or None on failure
        (missing ``pyngrok`` or
         no ``NGROK_AUTHTOKEN`` provided by user.
    ...
    try:
        ...
        logger.warning(
            ...
            "and restart the kernel to enable it. "
            "[/]"
        )
        return None
    ...
    if not os.environ.get("NGROK_AUTHTOKEN"):
        ...
        if env == "colab":
            try:
                ...
                from google.colab.userdata import (
                    SecretNotFoundError,  # type: ignore[import-untyped]
                )
        ...
        if token:
            ...
            token = input("NGROK_AUTHTOKEN (leave empty for more info): ").strip()
            if token:
                os.environ["NGROK_AUTHTOKEN"] = token
    ...
    try:
        ...
        msg = re.sub(r"(https?://\S+)", r"[#4d0066 on #FFD700 link=\1]\1[/]", msg)
...
def _webconsole_start_notebook(port: int, grpc_port: int) -> None:
    ...
    from .utils.executions.events import reset_for_restart as execs_reset_for_restart
    ...
    _ANSI_RE = re.compile(
        r"\x1b\[[0-9;]*[mAKHF]|\[[^\[\]\']*\]" + r"|[\u2500-\u257f]|[\U00010000-\U0010FFFF]"
    )
    ...
    if live_log_path:
        ...
        class _FlushingRotatingFileHandler(logging.handlers.TimedRotatingFileHandler):
            def emit(self, record):
                super().emit(record)
                self.flush()

        _live_handler = _FlushingRotatingFileHandler(
            live_log_path, when="midnight", backupCount=7, encoding="utf-8"
        )
    ...
    if _notebook_env() in ("colab", "kaggle"):
        ...
        if iframe_url is None:
            ...
            return
        display(HTML(f'<a href="{iframe_url}" target="_blank">{iframe_url}</a>'))
    else:
...
def _webconsole_shutdown_notebook():
    ...
    if not _main._server or not hasattr(_main._server, "should_exit"):
        display(HTML("<b>\N{WARNING SIGN} Server is not running " + "in this process.</b>"))
        return

    display(HTML("<b>\N{OCTAGONAL SIGN} Shutting down server...</b>"))
    ...
    from .utils.execution.events import notify_server_shutdown as exec_notify_server_shutdown
    from .utils.executions.events import notify_server_shutdown as execs_notify_server_shutdown
