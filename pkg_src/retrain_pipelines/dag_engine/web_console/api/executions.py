...
def register(app, rt, prefix=""):
    @rt_api(
        rt,
        url=f"{prefix}/api/v1/new_execution_event",
        methods=["POST"],
        schema={
            "requestBody": {
                "content": {
                    "application/json": {
                        "schema": {
                            ...
                            "required": ["id", "name", "username", "start_timestamp"],
                        }
                    }
                }
            },
    ...
    async def post_new_execution_event(request: Request):
    ...
    @rt_api(
        rt,
        url=f"{prefix}/api/v1/execution_end_event",
        methods=["POST"],
        schema={
            "requestBody": {
                "content": {
    ...
    async def post_execution_ended_event(request: Request):
