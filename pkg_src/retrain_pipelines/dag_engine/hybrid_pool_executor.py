...
class HybridPoolExecutor(Executor):
    ...
    def __init__(self, max_workers: int | None = None):
        ...
        max_workers : int
            ...
            (default: CPU count - 2).
        """
        self._max_workers = max(1, (max_workers or ((os.cpu_count() or 1) - 2)))
        self._process_pool = CloudpickleProcessPoolExecutor(
            max_workers=self._max_workers, mp_context=multiprocessing.get_context("fork")
        )
        self._thread_pool = ThreadPoolExecutor(
            max_workers=self._max_workers * 2
        )  # More threads for coordination
    ...
    def submit(self, fn: Callable[_P, _T], /, *args: _P.args, **kwargs: _P.kwargs) -> Future[_T]:
        ...
        -------
        Future
            represents the pending execution.
        """
        ...
        thread_future = self._thread_pool.submit(self._coordinate_task, fn, args, kwargs)

        return cast(Future[_T], thread_future)

    def _coordinate_task(self, fn: Callable[..., Any], args: tuple, kwargs: dict) -> Any:
...
def get_hybrid_executor(max_workers: int | None = None) -> HybridPoolExecutor:
