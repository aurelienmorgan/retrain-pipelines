...
from retrain_pipelines.utils.docker import print_container_log_tail
...
def parse_endpoint_response(model_name: str, raw_inference_request_items: list, port: int = 9080):
    # Create the request payload using Pydantic models
    request_data = InferenceRequest.model_validate([
        RawFeaturesRecord(data=item) for item in raw_inference_request_items
    ])
    ...
    if response.status_code == 200:
        try:
            parsed_response = InferenceResponse.model_validate_json(response.text).root
        ...
        print_container_log_tail(container_name=model_name, tail_length=20)
