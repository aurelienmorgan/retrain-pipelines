...
def preprocess_data_fct(
    ...
    for bucketize_feature in buckets:
        ...
        if isinstance(buckets[bucketize_feature], list):
            ...
            labels = [f"bucket{str(i + 1).zfill(num_digits)}" for i in range(num_buckets)]
            ...
            labels = [f"bucket{str(i + 1).zfill(num_digits)}" for i in range(bins)]
    ...
    # raw_features = X_raw.columns.values  # raw feature names
        ...
        if is_training:
            ...
            encoder_dict = {
                ...
                for feature, categories in zip(
                    categorical_features, encoder.categories_, strict=False
                )
            }
            ...
            # at inference time
            start_index = 0
            grouped_features = grouped_features or []
            for feature in categorical_features:
                num_categories = len(encoder_dict[feature])
                end_index = start_index + num_categories
                grouped_features.append(list(range(start_index, end_index)))
    ...
    if not numerical_features.empty:
        ...
        X_scaled = pd.DataFrame(
            ...
            columns=[str(f) + "_scaled" for f in numerical_features],
        )
    else:
        X_scaled = pd.DataFrame()
