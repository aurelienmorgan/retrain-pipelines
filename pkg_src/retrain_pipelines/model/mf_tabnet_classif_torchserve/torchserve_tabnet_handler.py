...
class TabnetHandler(BaseHandler):
    ...
    def initialize(self, context):
        ...
        if os.path.exists("feature_names.json"):
            with open("feature_names.json") as json_file:
        ...
        if os.path.exists("buckets_params.json"):
            with open("buckets_params.json") as json_file:
        ...
        with open("scaler_params.json") as json_file:
        ...
        with open("encoder_params.json") as json_file:
    ...
    def preprocess(self, data):
        ...
        if not isinstance(input_data, list):
            ...
            input_data = [request_input_item["data"] for request_input_item in input_data]

        input_data_df = pd.DataFrame(input_data, columns=self.raw_features)
        self.logger.debug(f"preprocess : {input_data_df}")

        # Apply the preprocessing function
        preprocessed_data = preprocess_data_fct(
            input_data_df, scaler=self.scaler, encoder=self.encoder, buckets=self.buckets
        )
