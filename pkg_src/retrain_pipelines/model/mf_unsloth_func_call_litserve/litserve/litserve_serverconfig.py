...
class Config:
    """Spec of a server configuration.
    ...
    with open("litserve_serverconfig.yaml") as file:
        yaml_config = yaml.safe_load(file)

    BASE_MODEL_PATH = (
        os.path.realpath(os.path.expanduser(BASE_MODEL_PATH))  # type: ignore[name-defined]
    ...
    if BASE_MODEL_PATH is not None:
        ...
        BASE_MODEL_REPO_ID = yaml_config["base_model"].get("repo_id") or "unsloth/Qwen2.5-1.5B"
        BASE_MODEL_REVISION = yaml_config["base_model"].get("revision") or None
        print(
            f"BASE_MODEL_REPO_ID : {BASE_MODEL_REPO_ID}, "
        ...
        HF_HUB_CACHE = os.path.realpath(
            os.path.expanduser(
                os.getenv(
                    "HF_HUB_CACHE",
                    os.path.join(os.getenv("HF_HOME", "~/.cache/huggingface"), "hub"),
                )
            )
        )
    ...
    if (yaml_adapters := yaml_config.get("adapters")) is not None:
        for adapter in yaml_adapters:
            ...
            if (adapter_path) is not None:
                adapter_path = os.path.realpath(os.path.expanduser(adapter_path))
                adapters[adapter["name"]] = {"path": adapter_path}
            else:
    ...
    MAX_SEQ_LENGTH = (
        int(MAX_SEQ_LENGTH)  # type: ignore[name-defined]
        if (MAX_SEQ_LENGTH := yaml_config.get("max_seq_length")) is not None
        else None
    )
    MAX_NEW_TOKENS = (
        int(MAX_NEW_TOKENS)  # type: ignore[name-defined]
        if (MAX_NEW_TOKENS := yaml_config.get("max_new_token")) is not None
        else None
    )

    PORT = int(PORT) if (PORT := yaml_config.get("port")) is not None else 8000  # type: ignore[name-defined]
    print(f"PORT : {PORT}")
