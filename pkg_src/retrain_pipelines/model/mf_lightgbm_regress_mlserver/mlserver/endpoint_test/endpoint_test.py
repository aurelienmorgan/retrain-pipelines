...
class InferenceRequest(BaseModel):
    ...
    def non_empty_inputs(cls, v):
        if not v:
            raise ValueError("Inference request must contain at least one input tensor.")
        return v
...
def parse_endpoint_response(
    ...
    try:
        request_data = InferenceRequest.parse_obj(raw_inference_request_items)
    ...
    if response.status_code == 200:
        try:
            ...
            if not outputs or "data" not in outputs[0]:
                raise ValueError("Missing 'outputs' or 'data' in response.")

            data = outputs[0]["data"]
            # flatten in case of [N,1]
            flattened = [float(x[0]) if isinstance(x, list) else float(x) for x in data]
            parsed_response = InferenceResponse.model_validate(flattened).root
            logger.debug(f"validated & parsed response: {parsed_response}")
        except ValueError as e:
