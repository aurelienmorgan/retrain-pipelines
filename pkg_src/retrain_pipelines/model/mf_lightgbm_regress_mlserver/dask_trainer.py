...
class MetricsCallback:
    def __init__(self, queue_name):
        ...
        For all Dask workers involved in a given model training.
        """
...
def dask_regressor_fit(
    ...
    """Fit a lightgbm regressor, relying on Dask distributed workers.
    ...
    npartitions : int
        number of partitions to apply for ditributed computation.
    hp_dict : dict
    ...
    ############################
    dX_train = dd.from_pandas(X_train, npartitions=npartitions)
    # for i in range(npartitions):
    #     partition_size = len(dX_train.get_partition(i))
    #     logger.info(f"Partition {i}: {partition_size} rows")
    dy_train = dd.from_pandas(y_train, npartitions=npartitions)
    ############################
    ...
    #     logger.info(f"Partition {i}: {partition_size} rows")
    dy_val = dd.from_pandas(y_val, npartitions=npartitions)
    ############################
    ...
    _dask_distrib_client = dask.distributed.Client(
    ...
    workers_history: dict[str, dict[str, list[float]]] = {}
    ...
    logger.debug("distributed best_score_ : " + f"{lgb_reg.best_score_['Validation']['rmse']}")

    return lgb_reg.to_local(), lgb_reg.evals_result_, workers_history
