...
os.environ["RP_ASSETS_CACHE"] = (
    os.environ.get("RP_ASSETS_CACHE", os.path.expanduser("~/.cache/retrain-pipelines/")).rstrip(
        os.sep
    )
    + os.sep
)
...
os.environ["RP_ARTIFACTS_STORE"] = (
    os.environ.get(
        "RP_ARTIFACTS_STORE",
        os.path.join(os.path.expanduser(os.environ["RP_ASSETS_CACHE"]), "artifacts"),
    ).rstrip(os.sep)
    + os.sep
)
try:
...
os.environ["RP_WEB_SERVER_PORT"] = os.environ.get("RP_WEB_SERVER_PORT", "5001")
os.environ["RP_GRPC_SERVER_PORT"] = os.environ.get("RP_GRPC_SERVER_PORT", "50051")
# serves for the internal dag-engine
os.environ["RP_WEB_SERVER_URL"] = os.environ.get(
    "RP_WEB_SERVER_URL", f"http://localhost:{os.environ['RP_WEB_SERVER_PORT']}/"
).rstrip("/")
...
os.environ["RP_WEB_SERVER_LOGS"] = (
    os.environ.get(
        "RP_WEB_SERVER_LOGS",
        os.path.join(os.path.expanduser(os.environ["RP_ASSETS_CACHE"]), "logs", "web_server"),
    ).rstrip(os.sep)
    + os.sep
)
try:
