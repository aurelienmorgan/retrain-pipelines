...
def _dimensions_from_df(input_df: pd.DataFrame) -> list:
        ...
        if (
            ...
            if input_df[col].dtype == "object":
                ...
                categ_dim = {
                    ...
                    "ticktext": col_categ.categories.unique().values.tolist(),
                    "label": col,
                ...
                if i != len(input_df.columns) - 1:
                    ...
                    num_dim = {"label": col, "values": input_df[col]}
...
def parallel_coord_plot(perf_df: pd.DataFrame) -> str:
    """Plot the parallel coordinate plot.

    For a given hyperparameters tuning settings.
    ...
    fig = go.Figure(
        data=go.Parcoords(
            line=dict(
                color=perf_df.iloc[:, -1:],
                colorscale="Portland",
                cmin=min(perf_df.iloc[:, -1]),
                cmax=max(perf_df.iloc[:, -1]),
                showscale=True,
                colorbar=dict(len=1, ypad=0, xpad=0, x=1, ticktext=[], tickvals=[]),
            ),
            dimensions=list(_dimensions_from_df(perf_df)),
            unselected=dict(line=dict(color="lightgrey", opacity=0.4)),
        )
    )
    ...
    fig.update_traces(tickfont_size=14, labelfont_size=14)
