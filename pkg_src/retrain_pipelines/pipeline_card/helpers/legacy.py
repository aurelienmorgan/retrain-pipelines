...
from graphviz import Digraph, Source  # type: ignore[import-not-found]
...
def mf_dag_svg(flow_spec: FlowSpec) -> str:
    ...
    for key in flow_graph_dict:
        ...
        if "join" == step["type"]:
            inner_foreach -= 1
        nodes.append((key, (inner_foreach > 0), textwrap.fill(step["doc"].replace("\n", " "), 50)))
    ...
    for _i, node in enumerate(nodes):
        ...
        if node[1]:
            nx.set_node_attributes(G, {node[0]: "box3d"}, name="shape")
            nx.set_node_attributes(G, {node[0]: "filled"}, name="style")
            nx.set_node_attributes(G, {node[0]: "orange"}, name="fillcolor")
            nx.set_node_attributes(G, {node[0]: "#e67e00"}, name="fontcolor")
            nx.set_node_attributes(
                G,
                {
                    node[0]: (
            ...
            nx.set_node_attributes(G, {node[0]: '"gold:brown"'}, name="fillcolor")
            nx.set_node_attributes(G, {node[0]: "radial"}, name="style")
            nx.set_node_attributes(G, {node[0]: "180"}, name="gradientangle")
    ...
    filter_elem = SubElement(defs, "filter", x="0", y="0", width="1", height="1", id="solid")

    SubElement(filter_elem, "feFlood", **{"flood-color": "yellow", "result": "bg"})
    ...
    graph_svg = tostring(root, encoding="utf-8", method="xml").decode("utf-8")
    #########################################
