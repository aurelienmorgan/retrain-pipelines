...
def get_html(params: Mapping[str, Any]) -> str:
    ...
    perf_metrics_df = perf_metrics_df.map(lambda x: f"{x:.3f}".rstrip("0").rstrip("."))
    perf_metrics_table = perf_metrics_df.to_html(classes="wide", escape=False, index=False)
    ...
    if params["pipeline_hp_grid_dict"]:
        ...
        hp_perfs_table = hp_perfs_df.sort_values(by="rmse", ascending=True).to_html(
            classes="wide", escape=False, index=False
        )
    else:
        hp_grid_table = None
    ...
    return template.render(
        ...
        hp_grid_table=indent(hp_grid_table, " " * 52) if hp_grid_table is not None else None,
        cv_folds=str(params["cv_folds"]),
        hp_perfs_curve=indent(hp_perfs_curve, " " * 60) if hp_perfs_curve is not None else None,
        hp_perfs_table=indent(hp_perfs_table, " " * 52) if hp_perfs_table is not None else None,
