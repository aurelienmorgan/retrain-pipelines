...
from retrain_pipelines.dataset.hf_utils import dataset_dict_to_config_str, get_size_category
...
def _dataset_readme_params(
    ...
    pipeline_name : str
        ...
        lead to generating the herein dataset repo README.
    exec_id : str
        execution id.
    engine : str
    ...
    enrich_commit_hash, enrich_commit_datetime = get_latest_README_commit(
    ...
    main_pretty_name = get_pretty_name(
        repo_id=hf_dataset_dict["repo_id"], repo_type="dataset", commit_hash=main_commit_hash
    )
    ...
    license_label = (
        ...
        if main_license_label == enrich_license_label
        or not (main_license_label and enrich_license_label)
        else None
    )
    if not license_label:
        license_label = "unknown"

    first_record_df = hf_dataset_dict["lazy_df"].limit(1).collect(engine=engine)[0]
    ...
    main_format_description = _build_keys(tool_0_0, output_str=main_format_description)
    ...
    return {
        ...
        "main_repo_id": hf_dataset_dict["repo_id"],
        "enrich_repo_id": hf_enrich_dataset_dict["repo_id"],
        ...
        "main_commit_datetime": main_commit_datetime,
        "enrich_commit_datetime": enrich_commit_datetime,
...
def get_dataset_readme_content(
    ...
    """Generate markdown for dataset card README.
    ...
    template_folder : str
        ...
        the dataset repo REAME template.
    hf_dataset_dict : dict
        - repo_id
        - commit_hash
        - commit_datetime
