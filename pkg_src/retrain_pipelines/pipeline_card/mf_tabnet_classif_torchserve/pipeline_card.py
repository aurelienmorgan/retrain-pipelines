...
from .helpers import (  # type: ignore[import-not-found]
    apply_args_color_format,
    fig_to_base64,
    highlight_min_max_cells,
    parallel_coord_plot,
)


def get_html(params: Mapping[str, Any]) -> str:
    """Generate the html for the portable pipeline card.

    At runtime for the DAG-execuction being run.
    """
    NONE_HTML_STR = "<center><font color=white><em>None</em></font></center>"
    DEFAULT_HTML_STR = "<center><font color=white><em>Default</em></font></center>"
    ...
    if not model_version_blessed:
        ...
        if not isinstance(current_blessed_exec, Execution):
            ...
            previous_blessed_model_card_task = [
                step.task for step in current_blessed_exec.steps() if step.id == "pipeline_card"
            ][0]
            previous_blessed_custom_card = cards.get_cards(
                previous_blessed_model_card_task, id="custom", type="html"
            )[0]

            current_blessed_run_finished = current_blessed_exec.finished_at.astimezone(
                pytz.utc
            ...
            current_blessed_run_finished = (  # LEGACY  -  RENAME VARIABLE ##
                current_blessed_exec.end_timestamp.astimezone(pytz.utc).strftime(  # type: ignore[union-attr]
                    "%A %b %d %Y %I:%M:%S %p %Z"
                )
            )
            previsous_blessed_card_href = f"/execution?id={current_blessed_exec.id}"
    ...
    features_desc_table = pd.DataFrame([params["features_desc"]]).to_html(
    ...
    data_heatmap_curve = fig_to_base64(data_heatmap_fig, extra_tight=True)
    ##########################
    ...
    if not params["buckets_dict"]:
        ...
        buckets_table = pd.DataFrame([params["buckets_dict"]]).to_html(
            classes="wide", escape=False, index=False
        )
    if params["hyperparameters_dict"]:
        hyperparameters_table = pd.DataFrame([params["hyperparameters_dict"]]).to_html(
            classes="wide", escape=False, index=False
        )
    else:
    ...
    for target_class, target_class_masks_fig in target_class_figs.items():
        if target_class_masks_fig is not None:
            ...
            target_class_curves[target_class] = fig_to_base64(
                target_class_masks_fig, extra_tight=True
            )
        else:
            target_class_curves[target_class] = None

    classes_weighted_metrics_df = pd.DataFrame([params["classes_weighted_metrics_dict"]])
    ...
    if params["sliced_perf_metrics_dict"] is None:
        ...
        sliced_perf_metrics_df = pd.DataFrame.from_dict(
            params["sliced_perf_metrics_dict"], orient="index"
        ).sort_index(ascending=True)
        styled_sliced_perf_metrics_table = highlight_min_max_cells(sliced_perf_metrics_df)
    ##########################
    ...
    if params["pipeline_hp_grid_dict"]:
        hp_grid_table = pd.DataFrame([params["pipeline_hp_grid_dict"]]).to_html(
            classes="wide", escape=False, index=False
        )

        hp_perfs_df = pd.DataFrame(params["hp_perfs_list"]).sort_values(
            by="accuracy", ascending=False
        )

        hp_perfs_curve = parallel_coord_plot(hp_perfs_df)

        hp_perfs_table = hp_perfs_df.to_html(classes="wide", escape=False, index=False)
        ...
        def add_fixed_column_class(html_string):
            ...
            if table:
                for row in table.find_all(["tr"]):
                    ...
                    last_cell["class"] = last_cell.get("class", []) + ["fixed-column"]
    ...
    return template.render(
        ...
        current_blessed_run_id=(  # LEGACY  -  RENAME VARIABLE ##
            None if model_version_blessed else current_blessed_exec.id
        ),
        current_blessed_run_finished=(
            None if model_version_blessed else current_blessed_run_finished
        ),
        previsous_blessed_card_href=(
            None if model_version_blessed else previsous_blessed_card_href
        ),
        previsous_blessed_card_url=(None if model_version_blessed else previsous_blessed_card_url),
        ###################################
        # infra validation status =>      #
        local_serve_color=(
            None
            if (-1 == local_serve_is_ready)
            else "#008000"
            if (1 == local_serve_is_ready)
            else "#811331"
        ),
        ...
        classes_weighted_metrics_table=indent(classes_weighted_metrics_table, " " * 52),
        slice_feature_name=slice_feature_name,
        styled_sliced_perf_metrics_table=(
            None
            if styled_sliced_perf_metrics_table is None
            else indent(styled_sliced_perf_metrics_table, " " * 52)
        ),
        hp_grid_table=indent(hp_grid_table, " " * 56) if hp_grid_table is not None else None,
        cv_folds=str(params["cv_folds"]),
        hp_perfs_curve=indent(hp_perfs_curve, " " * 60) if hp_perfs_curve is not None else None,
        hp_perfs_table=indent(hp_perfs_table, " " * 56) if hp_perfs_table is not None else None,
