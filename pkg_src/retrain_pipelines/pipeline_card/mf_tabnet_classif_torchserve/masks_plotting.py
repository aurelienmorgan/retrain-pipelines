...
def _tight_layout_manager(fig, **kwargs):
    try:
        ...
        if len(w) > 0 and issubclass(w[-1].category, UserWarning):
            print("Warning: tight_layout " + "may not be fully compatible " + "with all Axes.")
    except Exception as e:
        print(f"Error in tight_layout: {e}")


def _masks_column_idx(grouped_features: list, features_count: int) -> list:
    """Return list of feature indexes, first feature for each groups.
    ...
    categ_features_mask_column_idx = [
        inner_list[0] for inner_list in grouped_features if inner_list
    ]
    excluded_indices = {idx for sublist in grouped_features for idx in sublist}
    columns_to_keep = [
        idx
        for idx in range(features_count)
        if (idx in categ_features_mask_column_idx or idx not in excluded_indices)
    ]
...
def plot_masks_to_dict(
    ...
    """Return dict of mask plot per class.
    ...
    columns_to_keep = _masks_column_idx(grouped_features, X_transformed.shape[1])

    target_class_plots = {}
    target_classes = np.unique(y).tolist()
    for target_class in target_classes:
        target_class_idx = np.where(y == target_class)[0]
        predictions = model.predict(X_transformed.iloc[target_class_idx].values)
        target_class_true_positive_idx = target_class_idx[predictions == target_class]
        true_positives_display_count = min(
            max_true_positives_display_count, len(target_class_true_positive_idx)
        )
        print(
            f"subset of true positives for '{target_class}' : " + f"{true_positives_display_count}"
        )
        ...
        if true_positives_count > 0:
            ...
            displayed_masks_count = min(model.n_steps, max_displayed_masks_count)
            max_label_length = max(len(label) for label in raw_feature_names)
            # Adjust the below 2 to control figure height
            x_label_height_factor = 0.15
            plot_area_height_factor = 0.3 * (2 / displayed_masks_count) ** 3

            # Reserve space for title and subtitle
            title_space = 0.6  # inches
            fig_height = (
                plot_area_height_factor * true_positives_count
                + x_label_height_factor * max_label_length
                + title_space
            )
            fig, axs = plt.subplots(nrows=1, ncols=displayed_masks_count, figsize=(7, fig_height))
            ...
            for i, ax in enumerate(axs):
                im = ax.imshow(masks[i][:, columns_to_keep], cmap=cmap, norm=norm)
                ...
                ax.set_xticklabels(raw_feature_names, fontsize=9, rotation=45, ha="right")
            ...
            main_title = f"{first_masks_only}Masks " + f"for Class '{target_class}'"

            cbar_ax = fig.add_axes([0.85, 0.85, 0.1, 0.02])
            cbar = fig.colorbar(im, cax=cbar_ax, orientation="horizontal")
            cbar.set_label("Attention\nscore", fontsize=9, labelpad=-40, x=0.5, ha="center")
            cbar.ax.tick_params(labelsize=7)

            fig.suptitle(main_title, fontsize=11, y=1 - (0.1 * title_space / fig_height))
            # subtitle
            subtitle = "Number of true positives plotted: " + str(true_positives_display_count)
            fig.text(
                0.5, 1.03 - (0.8 * title_space / fig_height), subtitle, ha="center", fontsize=8
            )

            with _tight_layout_manager(
                fig,
                rect=[0, 0, 1, 1 - (title_space / fig_height)],
                pad=0.01,
                h_pad=0.01,
                w_pad=0.01,
            ):
