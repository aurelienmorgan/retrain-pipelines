...
abort_cmd = '("Command+." to abort)' if platform.system() == "Darwin" else '("Ctrl+C" to abort)'


def _find_env_python(env_name: str) -> str:
    """Return the fullpath to the python binary.
    ...
    def _find_venv_python(venv_name):
        cmd = f'find / -path "*/{venv_name}/bin/python"' + " -print -quit 2>/dev/null"
        try:
    ...
    try:
        result = subprocess.run(
            ["conda", "run", "-n", env_name, "which", "python"], capture_output=True, text=True
        )
...
def get_venv(virtual_env_name: str) -> dict[str, str]:
    """Return an instance of a virtual environment.
    ...
    dict[str, str]
    """
    print(f"Looking for virtual environment '{virtual_env_name}' " + f"{abort_cmd}.. ", end="")
    python_path = _find_env_python(virtual_env_name)
    assert python_path, "Virtual environment for this test is missing."
    print("Found.")
    ...
    path_dirs = [d for d in path_dirs if not os.path.exists(os.path.join(d, "python"))]
