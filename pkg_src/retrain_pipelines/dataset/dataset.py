...
def pseudo_random_generate(
    dataset_type: DatasetType, num_samples: int, seed: int | None = None
) -> pd.DataFrame:
    """Generate a pseudo-randomized dataset."""
    ...
    if dataset_type == DatasetType.TABULAR_REGRESSION:
        return _tabular_regression_generate(num_samples, seed)
    elif dataset_type == DatasetType.TABULAR_CLASSIFICATION:
        return _tabular_classification_generate(num_samples, seed)


def _tabular_regression_generate(num_samples: int, seed: int | None = None) -> pd.DataFrame:
    """
    Generate a pseudo-randomized tabular dataset with continous target variable.
    ...
    categ_feature0 = np.random.choice(
        categories, size=num_samples, p=np.random.dirichlet(np.ones(N), size=1).flatten()
    )
    ...
    correlation = 0.2
    # the uniform feature
    feature3 = np.random.uniform(low=low3, high=high3, size=num_samples)
    # the lognormal feature with a
    # slight correlation to the above uniform feature
    feature4 = np.exp(mean4 + sigma4 * np.random.normal(size=num_samples))
    feature4 = feature4 + correlation * (feature3 - feature3.mean()) / feature3.std()
    # normalize for mean and variance
    feature4 = (feature4 - feature4.mean()) / feature4.std() * sigma4 + mean4

    # random (some non-linear) transformations
    transformation_functions: list[Callable[..., np.ndarray]] = [
        cast(
            Callable[..., np.ndarray],
            random.choice([np.square, np.tanh, lambda x: np.exp(-(x**2)), lambda x: x]),
        )
        for _ in range(4)
    ]
    ...
    transformed_num_features = [
        ...
        for (func, feature) in zip(
            transformation_functions, [feature1, feature2, feature3, feature4], strict=False
        )
    ]
    ...
    normalized_num_features = [(f - np.mean(f)) / np.std(f) for f in transformed_num_features]
    combined_score = 25 * categorical_impact + np.sum(normalized_num_features, axis=0)

    # target variable based on transformed features
    # plus a tiny bit of noise
    raw_target = combined_score + np.random.normal(
        0, 1 / 2 * np.std(combined_score) * 5, num_samples
    )
    scaled_target = (raw_target - np.min(raw_target)) / (np.max(raw_target) - np.min(raw_target))
...
def _tabular_classification_generate(num_samples: int, seed: int | None = None) -> pd.DataFrame:
    """
    Generate a pseudo-randomized classification dataset (4 classes).
    ...
    # Combine the categorical feature impact
    # with the combination of the transformed numerical features
    combined_score = 5 * categorical_impact + np.sum(transformed_num_features, axis=0)

    # target variable based on transformed features
    # plus a tiny bit of noise
    raw_target = combined_score + np.random.normal(0, 1 / 3 * np.std(combined_score), num_samples)

    # 4 classes by splitting distrib of `raw_target`
    # in a "randomly" balanced manner
    random_percentiles = sorted([
        np.random.uniform(20 + i * 20 - 5, 20 + i * 20 + 5) for i in range(3)
    ])
    ...
    classes_priors = [round(100 * count / num_samples, 1) for count in np.bincount(target)]
    print(f"Classes priors: {classes_priors}%")

    return data


def features_desc(data: pd.DataFrame) -> dict:
    """
...
def features_distri_plot(data: pd.DataFrame) -> Figure:
    """Plot the respective distributions of the features and label.

    For all provided columns of the input dataset.
    Log-scale y-axis.
    ...
    for i, col in enumerate(columns):
        if col in categorical_columns:
            ...
            axes[i].set_xticklabels(sorted_values.index, rotation=45, ha="right")
        else:
    ...
    fig.tight_layout()  # plt.gcf().set_tight_layout(True)
