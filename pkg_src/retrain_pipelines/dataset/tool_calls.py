...
def _normalize_tool(tool: dict) -> dict:
    """Create a normalized version of a tool with consistent parameters ordering."""
...
def _parse_tools(tools_list_str: str) -> list[dict]:
    """Parse the 'list of tools' string into a list of normalized tools."""
    try:
...
def get_unique_tools(
    lazy_df: pl.LazyFrame, tools_attr_name: str, struct_schema: pl.Struct
) -> pl.LazyFrame:
    ...
    return (
        ...
        .with_columns([
            pl
            .col(tools_attr_name)
            .map_elements(_parse_tools, return_dtype=pl.List(struct_schema))
            .alias("parsed_tools")
        ])
        .select(pl.col("parsed_tools").explode().alias("tool"))
        .unique()
    )
...
def find_records_with_tool(
    ...
    """
    Find all records in the lazy DataFrame that contain a specific tool.
    ...
    def _contains_tool(tools_str: str) -> bool:
        try:
            ...
            return any(
                tool["name"] == lookup_tool["name"]
                and tool["description"] == lookup_tool["description"]
                and tool["parameters"] == lookup_tool["parameters"]
    ...
    return lazy_df.with_columns([
        ...
        .map_elements(_parse_tools, return_dtype=pl.List(struct_schema))
        .alias("parsed_tools")
    ]).filter(pl.col("tools").map_elements(_contains_tool, return_dtype=pl.Boolean))


def _normalize_answer_tool(tool: dict) -> dict:
    """Create a normalized version of a tool with consistent parameter ordering."""
    normalized = tool.copy()
    if "arguments" in normalized:
        # If parameters is already a list of string
        # (from previous processing), parse it first
        if isinstance(normalized["arguments"], list) and all(
            isinstance(i, str) for i in normalized["arguments"]
        ):
            try:
                params = json.loads(str(normalized["arguments"]))
            except json.JSONDecodeError:
                try:
                    params = literal_eval(str(normalized["arguments"]))
                except (ValueError, SyntaxError) as ex:
                    print(ex, file=sys.stderr)
                    print(tool, file=sys.stderr)
                    params = []
...
def _parse_answer_tools(tools_list_str: str) -> list[dict]:
    """Parse the 'list of tools' string.

    Into a list of normalized tools.
    ...
    try:
        tools_list = json.loads(tools_list_str)
        return [_normalize_answer_tool(tool) for tool in tools_list]
    except Exception as ex:
        print(tools_list_str, file=sys.stderr)
        print(ex, file=sys.stderr)
        return []
...
def count_tool_occurrences(
    lazy_df: pl.lazyframe.frame.LazyFrame, column_name: str, struct_schema: pl.Struct
    ...
    for field in struct_schema.fields:
        with_columns.append(
            pl.col("tool").struct.field(field.name).alias(f"tool_{field.name}"),
        )
    ...
    if "parameters" == param_arg:
        ...
        raise ValueError(f"Unexpected value for 'param_arg': {param_arg}")
    ...
    return (
        ...
        .with_columns([
            pl
            .col(column_name)
            .map_elements(tool_parsing_method, return_dtype=pl.List(struct_schema))
            .alias("parsed_tools")
        ])
        .select(pl.col("parsed_tools").explode().alias("tool"))
        .filter(pl.col("tool").is_not_null())
        .group_by("tool")
...
def _mpl_float_format_func(value, tick_number):
    """Generate Matplotlib Formatter for thousand-separated ints.
...
def plot_tools_occurences(
    ...
    """Plot distribution of tools within set of records.
    ...
    _bars1 = ax.bar(
        x_coords[:head_tail_size], head["occurrences"], color="skyblue", label="most frequent"
    )
    _bars2 = ax.bar(
        x_coords[head_tail_size:], tail["occurrences"], color="lightsalmon", label="least frequent"
    )

    # x-axis
    all_labels = pl.concat([head["tool_name"], tail["tool_name"]])
    ...
    ax.yaxis.set_major_formatter(FuncFormatter(_mpl_float_format_func))
    ax.tick_params(axis="y", labelsize=fontsize)

    ax.set_xlabel("Tool Name (every 10th)", fontsize=fontsize)
    ax.set_title(
        title_prefix + f"Top {head_tail_size} and Bottom {head_tail_size} " + "Tool Occurrences",
        fontsize=fontsize + 2,
    )

    # gap band
    ax.axvspan(head_tail_size, head_tail_size + 50, facecolor="lightgray", alpha=0.5)
    gap_text = f"{len(tools_occurences_df) - 2 * head_tail_size:,.0f} " + "entries\nnot shown"
    ax.text(
        head_tail_size + 25,
        0.5,
        gap_text,
...
def column_words_stats(
    ...
    """Generate DataFrame of queries words-count quantiles.
    ...
    column_attr_handler : Callable, optional
        ...
        Usage:
            ...
            Schema([('question', Struct({'text': String,
                                         'tokens': List(String)}))
            words_stats =             column_words_stats(
                    polars_df,
...
def plot_words_count(
    ...
    """Generate and plot words-count horizontal barchart.
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(7, 3.78), gridspec_kw={"width_ratios": [2, 1]})
    ...
    for count in word_counts:
        for i, (lower, upper) in enumerate(bin_ranges):
            if lower < count <= upper:
                binned_counts[i] += 1
                break
    ax1.barh(range(len(binned_counts)), binned_counts, color="skyblue", edgecolor="black")

    ax1.set_ylabel("Words Count Range", fontsize=fontsize)
    ax1.set_yticks(range(len(binned_counts)))
    ax1.set_yticklabels([f"{lower + 1}-{upper}" for lower, upper in bin_ranges], ha="right")
    ax1.tick_params(axis="y", labelsize=fontsize - 1)

    ax1.set_xlabel("Number of Records in Range (log scale)", fontsize=fontsize)
    ax1.set_xscale("log")
    ax1.xaxis.set_major_formatter(FuncFormatter(_mpl_int_format_func))
