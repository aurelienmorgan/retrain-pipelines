...
from .launcher import dag_help, dag_render, launch


def _tokenize(command: str) -> list:
    """Split a magic command line into tokens.

    Keeping brace-delimited dict literals intact
    as single tokens.
    """
    tokens = []
    i = 0
...
def _make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--exec-params", dest="exec_params", required=False)
    parser.add_argument("--exec-workers", dest="exec_workers", required=False)
    return parser


def _resolve_path_str(raw: str, ns: dict) -> str:
    """Turn "Variable name" (if exists) into its string value.
    ...
    if raw in ns:
        ...
        if not isinstance(value, str):
            raise TypeError(
                f"%retrain_pipelines: {raw!r} must resolve to a str, got {type(value).__name__}"
            )
        return value
...
def _validate_path(path: str) -> str | None:
    """Resolve to an absolute real path and verify the file exists.

    Returns the resolved path on success, prints to stderr and returns
    None on failure.
    """
    path = os.path.realpath(path)
    if not os.path.exists(path):
        print(f"Pipeline file not found: {path}", file=sys.stderr, flush=True)
        return None
    return path
...
def _resolve_dict(raw: str, ns: dict) -> dict:
    ...
    try:
        ...
        raise ValueError(f"%retrain_pipelines: could not evaluate {raw!r}") from e
    if not isinstance(value, dict):
        raise TypeError(f"%retrain_pipelines: expected a dict, got {type(value).__name__}")
    return value
...
def retrain_pipelines(command: str) -> None:
    ...
    ip = get_ipython()
    if ip is None:
        raise RuntimeError("%retrain_pipelines must be run " + "inside a Jupyter environment.")
    ...
    if args.action == "help":
        filefullname = _validate_path(_resolve_path_str(args.filefullname, ip.user_ns))
    ...
    filefullname = _validate_path(_resolve_path_str(args.filefullname, ns))
    if filefullname is None:
        return
    # restart_from = args.restart_from  # TODO
    exec_params = _resolve_dict(args.exec_params, ns) if args.exec_params is not None else None
    # exec_workers = (                  # TODO
    # _resolve_dict(args.exec_workers, ns) if args.exec_workers is not None else None
    # )
    ######################################################
...
def load_ipython_extension(ipython):
    ...
    """
    ipython.register_magic_function(retrain_pipelines, magic_kind="line")
