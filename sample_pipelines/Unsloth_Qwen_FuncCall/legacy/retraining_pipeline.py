...
from metaflow.cards import Image, Markdown, Artifact, get_cards
...
class UnslothFuncCallFlow(FlowSpec):
    ...
    cpt_training_args = Parameter(
        "cpt_training_args",
        help="dict with `TrainingArguments` params " + "for the CPT job.",
        type=JSONType,
        default=dedent("""{
            "warmup_ratio": 0.1,
            "num_train_epochs": 1
    ...
    sft_training_args = Parameter(
        "sft_training_args",
        help="dict with `TrainingArguments` params " + "for the SFT job.",
        type=JSONType,
        default=dedent("""{
            "warmup_ratio": 0.1,
            "num_train_epochs": 1
    ...
    def copy_default_dataset_readme_module(
        ...
        if not exists_ok and os.path.exists(
            os.path.join(target_dir, "dataset_readme.py")
        ):
    ...
    def copy_default_dataset_readme_template(
        ...
        os.makedirs(target_dir, exist_ok=True)
        if not exists_ok and os.path.exists(
            os.path.join(target_dir, "dataset_readme_template.md")
        ):
            print("File already exists. Skipping copy.")
        else:
            filefullname = os.path.join(
    ...
    def copy_default_model_readme_module(
        ...
        os.makedirs(target_dir, exist_ok=True)
        if not exists_ok and os.path.exists(
            os.path.join(target_dir, "model_readme.py")
        ):
            print("File already exists. Skipping copy.")
        else:
            filefullname = os.path.join(
                UnslothFuncCallFlow.default_pipeline_card_module_dir, "model_readme.py"
            )
            shutil.copy(filefullname, target_dir)
            print(filefullname)
    ...
    def copy_default_model_readme_template(
        ...
        os.makedirs(target_dir, exist_ok=True)
        if not exists_ok and os.path.exists(
            os.path.join(target_dir, "model_readme_template.md")
        ):
            print("File already exists. Skipping copy.")
        else:
            filefullname = os.path.join(
    ...
    def copy_default_pipeline_card_module(
        ...
        if not exists_ok and os.path.exists(
            ...
            filefullname = os.path.join(
                UnslothFuncCallFlow.default_pipeline_card_module_dir, "pipeline_card.py"
            )
            shutil.copy(filefullname, target_dir)
            print(filefullname)
    ...
    def copy_default_pipeline_card_html_template(
        ...
        if not exists_ok and os.path.exists(os.path.join(target_dir, "template.html")):
            ...
            filefullname = os.path.join(
                UnslothFuncCallFlow.default_pipeline_card_module_dir, "template.html"
            )
            shutil.copy(filefullname, target_dir)
            print(filefullname)
    ...
    def start(self):
        ...
        self.engine = (
            "cpu"
            if (self.polars_engine == "gpu" and not torch.cuda.is_available())
            else self.polars_engine
        )

        # hf_dataset
        hf_dataset_dict = get_lazy_df(
            repo_id=self.hf_dataset["repo_id"],
            commit_hash=self.hf_dataset["commit_hash"],
            config_name=(
                self.hf_dataset["config_name"] and "" < self.hf_dataset["config_name"]
            ),
        ...
        try:
            ...
            if "HF_TOKEN" not in os.environ:
                print(
                    "Does the Hugging Face-hosted dataset "
        ...
        print(
            " ; ".join(
                f"{k}: {hf_enrich_dataset_dict[k]}"
                for k in ["commit_hash", "commit_datetime"]
            )
        )
        ...
        hf_base_model_revision = (
            None
            if (rev_commit_hash := self.hf_base_model["commit_hash"]) == ""
        ...
        self.hf_base_model_dict = {
            ...
            "commit_datetime": hf_base_model_commit.created_at,
        }
        ...
        self.unsloth_dir = os.path.join(self.serving_artifacts_local_folder, "Unsloth")
        print(f"unsloth_dir : {self.unsloth_dir}")
        self.cpt_model_dir = os.path.join(self.unsloth_dir, "cpt_model")
        self.sft_model_dir = os.path.join(self.unsloth_dir, "sft_model")
    ...
    def eda(self):
        ...
        tool_answer_occurrences_df = count_tool_occurrences(
            self.hf_dataset_dict["lazy_df"],
            self.hf_dataset["attributes"]["answers_attr"],
            struct_schema,
        ).collect(engine=self.engine)
        print(
            f"{tool_answer_occurrences_df['occurrences'].sum():,} "
            + "query/tool-calls pairs"
        )
        fig = plot_tools_occurences(
            tool_answer_occurrences_df, title_prefix="Dataset answers - "
        )
        ...
        queries_max_length = (
            self.hf_dataset_dict["lazy_df"]
            .select(
                pl.col(self.hf_dataset["attributes"]["query_attr"])
                .str.len_chars()
                .max()
        ...
        print(
            "longuest query counts "
            + f"{queries_max_length['max_query_length'][0]:,} characters"
        )

        # queries length quartiles
        self.query_words_stats = column_words_stats(
            self.hf_dataset_dict["lazy_df"], self.hf_dataset["attributes"]["query_attr"]
        ).collect(engine=self.engine)
        print(self.query_words_stats.to_pandas().to_string(index=False))
        print(
            "Two thirds of the records have a query with less than "
        ...
        #    Query words count     #
        ############################
        enrich_question_words_stats = column_words_stats(
            self.hf_enrich_dataset_dict["lazy_df"],
            self.hf_enrich_dataset["query_attribute"],
            column_attr_handler=eval(self.hf_enrich_dataset["query_attribute_handler"]),
        ).collect(engine=self.engine)
        print(enrich_question_words_stats.to_pandas().to_string(index=False))
    ...
    def augment_data(self):
        ...
        print(
            "Sampling within the population with more than "
            + str(self.query_words_stats["q3"][0])
            + " words (longest queries quartile) =>"
        )

        samples_count = int(self.records_count * self.augmentation_rate)
        print(f"would represent {samples_count:,.0f} " + "records to be sampled")
        ...
        eligible_records_df = (
            ...
            .filter(
                ...
                & pl.col("answers").map_elements(
                    lambda x: len(json.loads(x)) == 1 if isinstance(x, str) else False,
                    return_dtype=pl.Boolean,
                )
            )
            .collect(engine=self.engine)
        )
        eligible_records_count = eligible_records_df.select(pl.len())["len"][0]
        print("eligible_records_count : " + f"{eligible_records_count:,.0f}")
        samples_count = min(samples_count, eligible_records_count)
        self.actual_augmentation_rate = samples_count / self.records_count
        print("actual augmentation rate : " + f"{self.actual_augmentation_rate:.1%}")
        sampled_records_df = eligible_records_df.sample(n=samples_count)

        self.augmented_records_df = sampled_records_df.with_columns(
            pl.col("query")
            .map_elements(
                lambda query: " ".join(query.split()[: len(query.split()) // 2]),
                return_dtype=pl.Utf8,
            )
            .alias("truncated_query")
        ).select(
            [pl.col("truncated_query").alias("query"), pl.lit("[]").alias("answers")]
        )
        print(self.augmented_records_df.height, self.augmented_records_df.columns)
    ...
    def enrich_data(self):
        ...
        samples_count = int(self.records_count * self.enrichment_rate)
        print(f"Samplig {samples_count:,.0f} records")

        query_attribute_handler = eval(
        ...
        print(
            "Starting sample enriching records, "
            + "this may take some time if the source dataset "
            + "has a complex structure.."
        )
        samples_list = [
            s.capitalize() + ("" if s[-1] in ".!?" else "?") for s in samples_iterator
        ]
        ...
        enriched_records_df = pl.DataFrame(
            {"query": samples_list, "answers": ["[]"] * len(samples_list)}
        )
    ...
    def dataset_to_hub(self):
        ...
        # 'pipeline_card_artifacts_path'
        if "dataset_readme_template.md" in os.listdir(
        ...
        #############################
        if "dataset_readme.py" in os.listdir(self.pipeline_card_artifacts_path):
            from retrain_pipelines.utils import get_get_dataset_readme_content

            get_dataset_readme_content = get_get_dataset_readme_content(
                self.pipeline_card_artifacts_path
            )
        else:
            from retrain_pipelines.pipeline_card import get_dataset_readme_content
        #############################
        ...
        merged_df.rechunk()
        print(("merged_df", f"{merged_df.shape[0]:,.0F}", merged_df.columns))
        ...
        unique_tools_df = get_unique_tools(
            self.hf_dataset_dict["lazy_df"],
            tools_attr_name=self.hf_dataset["attributes"]["tools_attr"],
            struct_schema=struct_schema,
        ).collect(engine=self.engine)
        unique_tools_arrow_table = unique_tools_df.to_arrow()
        self.unique_tools_dataset = Dataset(unique_tools_arrow_table)
        ...
        dataset_dict = DatasetDict(
            {
                "continued_pre_training": self.unique_tools_dataset,
        ...
        dataset_commit_hash = push_dataset_version_to_hub(
            repo_id=self.dataset_repo_id,
            version_label=new_dataset_version_label,
            timestamp_str=commit_datetime.strftime("%Y-%m-%d %H:%M:%S UTC"),
            dataset_dict=dataset_dict,
            dataset_readme_content=readme_content,
            hf_token=os.getenv("HF_TOKEN", None),
        )
        if not dataset_commit_hash:
            raise Exception("Failed to publish dataset version.")
        print(
            f"https://huggingface.co/datasets/{self.dataset_repo_id}"
    ...
    def continued_pre_training(self):
        ...
        #######################################
        tools_dataset = DatasetDict({"train": self.unique_tools_dataset})
        print(tools_dataset)
        ...
        def formatting_prompts_func(tools_batch):
            ...
            for tool in tools_batch:
                ...
                text = tool_prompt_template.format(tool) + tokenizer.eos_token
                outputs.append(text)
            return {
                "tools": outputs,
            }
        ...
        #######################################
        if (
            "records_cap" in self.cpt_training_args
            and self.cpt_training_args["records_cap"] is not None
            and isinstance(self.cpt_training_args["records_cap"], int)
        ):
            cpt_dataset = cpt_dataset.take(self.cpt_training_args["records_cap"])
        ...
        train_args = UnslothTrainingArguments(
            ...
            **{k: v for k, v in self.cpt_training_args.items() if k != "records_cap"},
            ...
            # seed=3407,
            output_dir=os.path.join(self.unsloth_dir, "outputs", "cpt"),
            save_total_limit=2,
            report_to="tensorboard",
            logging_dir=os.path.join(self.sft_model_dir, "runs", "cpt"),
        )
        ...
        print(f"GPU = {gpu_stats.name}. " + f"Max memory = {self.max_memory} GB.")
        ...
        self.cpt_log_history_fig = plot_log_history(
            self.cpt_log_history, title="Continued pretraining loss"
        )

        model.save_pretrained_merged(
            save_directory=self.cpt_model_dir, tokenizer=tokenizer, save_method="lora"
        )
    ...
    def supervised_finetuning(self):
        ...
        def formatting_prompts_func(records):
            ...
            for query, tools in zip(query, tools):
                ...
                text = self.sft_prompt_template.format(query, tools) + EOS_TOKEN
                outputs.append(text)
            return {
                "text": outputs,
            }
        ...
        if (
            "records_cap" in self.sft_training_args
            and self.sft_training_args["records_cap"] is not None
            and isinstance(self.sft_training_args["records_cap"], int)
        ):
            train_dataset = train_dataset.take(self.sft_training_args["records_cap"])
            eval_dataset = eval_dataset.take(self.sft_training_args["records_cap"])
        ...
        train_args = UnslothTrainingArguments(
            per_device_train_batch_size=2,
            gradient_accumulation_steps=8,
            **{k: v for k, v in self.sft_training_args.items() if k != "records_cap"},
            ...
            lr_scheduler_type="linear",
            # seed=3407,
            output_dir=os.path.join(self.unsloth_dir, "outputs", "sft"),
            save_total_limit=2,
            disable_tqdm=True,
            logging_steps=1,
            report_to="tensorboard",
            logging_dir=os.path.join(self.sft_model_dir, "runs", "sft"),
        )
        ...
        print("Peak reserved memory = " + f"{used_memory} GB.")
        print(
            "Peak reserved memory for " + f"training = {used_memory_for_lora} " + "GB."
        )
        print("Peak reserved memory % of " + f"max memory = {used_percentage} %.")
        print(
        ...
        self.sft_log_history_fig = plot_log_history(
            self.sft_log_history, title="Supervised finetuning loss"
        )

        model.save_pretrained_merged(self.sft_model_dir, tokenizer, save_method="lora")
    ...
    def evaluate_model(self):
        ...
        validation_results = infer_validation(
            ...
            batch_size=32,  # 64,
            queries_attr_name=self.hf_dataset["attributes"]["query_attr"],
            answers_attr_name=self.hf_dataset["attributes"]["answers_attr"],
        ...
        records = eval_df.with_columns(
            (pl.col("answer") == pl.col("completion")).alias(
                "is_ground_truth_identical"
            )
        ).collect()  # engine=self.engine)
        print(
            "perfect characters-match accuracy : "
        ...
        self.validation_completions_fig = plot_validation_completions(
    ...
    def model_version_blessing(self):
        ...
        """
        from retrain_pipelines.model.hf_utils import current_blessed_model_version_dict

        main_perf_metric_name = "jaccard"

        current_blessed_version_dict = current_blessed_model_version_dict(
            repo_id=self.model_repo_id, hf_token=os.getenv("HF_TOKEN", None)
        )
        print("current_blessed_version_dict : " + str(current_blessed_version_dict))
        ...
        if current_blessed_version_dict is None:
        ...
        elif main_perf_metric_name in current_blessed_version_dict["perf_metrics"]:
            current_blessed_run_id = current_blessed_version_dict["exec_id"]
            print(f"current_blessed_run_id : {current_blessed_run_id}")
            current_blessed_metric_value = current_blessed_version_dict["perf_metrics"][
                main_perf_metric_name
            ]

            self.model_version_blessed = (
                self.perf_metrics[main_perf_metric_name] >= current_blessed_metric_value
            )

            # self.model_version_blessed = False ### DEBUG - DELETE ###

            if not self.model_version_blessed:
                self.current_blessed_version_dict = current_blessed_version_dict
                    ...
                    if str(run.id) == current_blessed_run_id:
                        ...
                        if not run_has_custom_card_artifact:
                            ...
                            if (
                                ...
                                # and been resumed
                                hasattr(last_task.artifacts, "model_version_blessed")
                                and last_task.artifacts.model_version_blessed.data
                                and hasattr(last_task.artifacts, "model_repo_id")
                                and last_task.artifacts.model_repo_id.data
                                == self.model_repo_id
                            ):
            ...
            raise Exception(
                ...
                + "from blessed run "
                + str(current_blessed_version_dict["exec_id"])
                + " !"
            )
    ...
    def model_to_hub(self):
        ...
        # when specifying custom
        # 'pipeline_card_artifacts_path'
        if "model_readme_template.md" in os.listdir(self.pipeline_card_artifacts_path):
            template_dir = self.pipeline_card_artifacts_path
        else:
            template_dir = os.path.dirname(
                importlib.util.find_spec(
        ...
        print(f"template_dir : '{template_dir}'")
        #############################
        if "model_readme.py" in os.listdir(self.pipeline_card_artifacts_path):
            from retrain_pipelines.utils import get_get_model_readme_content

            get_model_readme_content = get_get_model_readme_content(
                self.pipeline_card_artifacts_path
            )
        else:
            from retrain_pipelines.pipeline_card import get_model_readme_content
        #############################
        from retrain_pipelines.model.hf_utils import push_model_version_to_hub
        ...
        print(
            "Pushing model version to HF hub "
            + ("(blessed). " if self.model_version_blessed else "(not blessed). ")
            + "May take a while..",
            flush=True,
        )
        ...
        if not model_commit_hash:
            raise Exception("Failed to publish model version.")
        print("Push of model version to HF hub completed.", flush=True)
        print(
            f"https://huggingface.co/{self.model_repo_id}"
    ...
    def infra_validator(self):
        ...
        if self.model_version_blessed:
            from retrain_pipelines.utils.docker import env_has_docker

            if env_has_docker():
                model_module_dir = os.path.dirname(
                    importlib.util.find_spec(
                        "retrain_pipelines.model." + os.getenv("retrain_pipeline_type")
                    ).origin
                )
                ...
                for filename in files_to_copy:
                    shutil.copy(
                        ...
                        os.path.join(self.serving_artifacts_local_folder, filename),
                    )
                ...
                # server config yaml
                env = Environment(
                    loader=FileSystemLoader(os.path.join(model_module_dir, "litserve"))
                )
                template = env.get_template("litserve_serverconfig_template.yaml")
                server_config_data = {
                    "port": "8000",
                    "max_seq_length": self.max_seq_length,
                    ...
                    "adapters": [{"name": "func_caller", "path": "/FuncCallAdapter"}],
                }
                ...
                # Dockerfile
                env = Environment(
                    loader=FileSystemLoader(os.path.join(model_module_dir))
                )
                template = env.get_template("Dockerfile.litserve_template")
                ...
                with open(
                    os.path.join(
                        self.serving_artifacts_local_folder, "Dockerfile.litserve"
                    ),
                    "w",
                ) as output_file:
                ...
                self.port = 8765
                HF_HUB_CACHE = os.path.realpath(
                    os.path.expanduser(
                        os.getenv(
                            "HF_HUB_CACHE",
                            os.path.join(
                                os.getenv("HF_HOME", "~/.cache/huggingface"), "hub"
                            ),
                        )
                    )
                )
                ...
                serving_container = build_and_run_docker(
                    ...
                    volumes_dict={
                        self.sft_model_dir: {"bind": "/FuncCallAdapter", "mode": "ro"},
                ...
                self.local_serve_is_ready = LocalServeReadinessEnum.FAILURE_NO_DOCKER

            if LocalServeReadinessEnum.SUCCESS == self.local_serve_is_ready:
                from retrain_pipelines.model.litserve.litserve_datamodel import Response
                ...
                try:
                    ...
                    elapsed_time = time.time() - start_time
                    print(
                        "parsed_response ('func_caller' adapter ON) :"
                        + str(parsed_response)
                        + f"\t-\tElapsed time: {elapsed_time:.2f} seconds"
                    )

                    start_time = time.time()
                    data = {"queries_list": '["Hello.", "Is 49 a perfect square?"]'}
                    print(f"inference test - data: {data}")
                    response = requests.post(url, headers=headers, data=data)
                    parsed_response = Response(**{"output": response.json()})
                    elapsed_time = time.time() - start_time
                    print(
                        f"parsed_response (no adapter) : {parsed_response}"
                        + f"\t-\tElapsed time: {elapsed_time:.2f} seconds"
                    )
    ...
    def pipeline_card(self):
        ...
        self.metadata = {
            ...
            "references": [
                ...
                {"title": "Unsloth", "link": "https://unsloth.ai/blog/contpretraining"},
            ],
        }
        ...
        current.card["default"].append(Image.from_matplotlib(self.sft_log_history_fig))
        current.card["default"].append(
        ...
        params = {
            ...
            "main_dataset_commit_datetime": self.hf_dataset_dict["commit_datetime"],
    ...
    def pipeline_to_hub(self):
        ...
        subfolder_name = (
            "v" + self.model_commit_dict["version_label"] + "_" + timestamp_str
        )
        commit_datetime = datetime.utcnow()
        ...
        if self.pipeline_card_artifacts_path != self.default_pipeline_card_module_dir:
            ...
            for candidate_source_file in candidate_source_files:
                file_fullpath = os.path.join(
                    self.pipeline_card_artifacts_path, candidate_source_file
                )
                if os.path.exists(file_fullpath):
                    custom_source_files.append(file_fullpath)

        source_code_commit_hash = push_files_to_hub_repo_branch(
            repo_id=self.model_repo_id,
            ...
            commit_message="source-code for model version "
            + subfolder_name
    ...
    def deploy(self):
        ...
        if self.model_version_blessed and (
            self.local_serve_is_ready == LocalServeReadinessEnum.SUCCESS
        ):
            pass  # your code here
