...
def start() -> TaskPayload:
    ...
    ctx.engine = (
        "cpu"
        if (ctx.polars_engine == "gpu" and not torch.cuda.is_available())
    ...
    hf_dataset_dict = get_lazy_df(
        ...
        config_name=(
            ctx.hf_dataset["config_name"] and "" < ctx.hf_dataset["config_name"]
        ),
    ...
    try:
        ...
        if "HF_TOKEN" not in os.environ:
            logger.info(
                "Does the Hugging Face-hosted dataset " + "require authentication ?",
                file=sys.stderr,
                flush=True,
            )
        raise ex
    ...
    hf_enrich_dataset_dict = get_lazy_df(
        ...
        config_name=(
            ...
            and "" < ctx.hf_enrich_dataset["config_name"]
        ),
        hf_token=os.getenv("HF_TOKEN", None),
    )
    ...
    hf_base_model_revision = (
        None
        if (rev_commit_hash := ctx.hf_base_model["commit_hash"]) == ""
...
def eda(_) -> None:
    ...
    ctx.data_schema = get_column_info(ctx.hf_dataset_dict["lazy_df"], engine=ctx.engine)
    ############################
    ...
    queries_max_length = (
        ctx.hf_dataset_dict["lazy_df"]
        .select(
            pl.col(ctx.hf_dataset["attributes"]["query_attr"])
            .str.len_chars()
            .max()
    ...
    ctx.query_words_stats = column_words_stats(
        ctx.hf_dataset_dict["lazy_df"], ctx.hf_dataset["attributes"]["query_attr"]
    ).collect(engine=ctx.engine)
    ...
    enrich_question_words_stats = column_words_stats(
        ...
        column_attr_handler=eval(ctx.hf_enrich_dataset["query_attribute_handler"]),
    ).collect(engine=ctx.engine)
...
def augment_data(_) -> None:
    ...
    samples_count = int(ctx.records_count * ctx.augmentation_rate)
    print(
        f"{ctx.augmentation_rate:.1%} would represent "
    ...
    ctx.actual_augmentation_rate = samples_count / ctx.records_count
    print("actual augmentation rate : " + f"{ctx.actual_augmentation_rate:.1%}")
    sampled_records_df = eligible_records_df.sample(n=samples_count)

    ctx.augmented_records_df = sampled_records_df.with_columns(
        pl.col("query")
        .map_elements(
    ...
    ).select([pl.col("truncated_query").alias("query"), pl.lit("[]").alias("answers")])
    print(ctx.augmented_records_df.height, ctx.augmented_records_df.columns)
...
def enrich_data(_) -> None:
    ...
    samples_count = int(ctx.records_count * ctx.enrichment_rate)
    print(f"Samplig {samples_count:,.0f} records")

    query_attribute_handler = eval(ctx.hf_enrich_dataset["query_attribute_handler"])
...
def dataset_to_hub(_) -> None:
    ...
    if "dataset_readme_template.md" in os.listdir(ctx.pipeline_card_artifacts_path):
    ...
    #############################
    if "dataset_readme.py" in os.listdir(ctx.pipeline_card_artifacts_path):
    ...
    unique_tools_df = get_unique_tools(
        ctx.hf_dataset_dict["lazy_df"],
        tools_attr_name=ctx.hf_dataset["attributes"]["tools_attr"],
        struct_schema=struct_schema,
    ).collect(engine=ctx.engine)
    unique_tools_arrow_table = unique_tools_df.to_arrow()
    ctx.unique_tools_dataset = Dataset(unique_tools_arrow_table)
    ...
    dataset_dict = DatasetDict(
        {
            "continued_pre_training": ctx.unique_tools_dataset,
    ...
    print(
        f"https://huggingface.co/datasets/{ctx.dataset_repo_id}"
...
def continued_pre_training(_) -> None:
    ...
    #######################################
    tools_dataset = DatasetDict({"train": ctx.unique_tools_dataset})
    print(tools_dataset)
    ...
    #######################################
    if (
        "records_cap" in ctx.cpt_training_args
        and ctx.cpt_training_args["records_cap"] is not None
        and isinstance(ctx.cpt_training_args["records_cap"], int)
    ):
    ...
    train_args = UnslothTrainingArguments(
        ...
        **{k: v for k, v in ctx.cpt_training_args.items() if k != "records_cap"},
    ...
    ctx.start_gpu_memory = round(
        torch.cuda.max_memory_reserved() / 1024 / 1024 / 1024, 3
    )
    ctx.max_memory = round(gpu_stats.total_memory / 1024 / 1024 / 1024, 3)
    print(f"GPU = {gpu_stats.name}. " + f"Max memory = {ctx.max_memory} GB.")
    ...
    ctx.cpt_log_history_fig = plot_log_history(
        ctx.cpt_log_history, title="Continued pretraining loss"
    )
    del trainer
...
def supervised_finetuning(_) -> None:
    ...
    def formatting_prompts_func(records):
        ...
        for query, tools in zip(query, tools):
            ...
            text = ctx.sft_prompt_template.format(query, tools) + EOS_TOKEN
            outputs.append(text)
        return {
            "text": outputs,
        }
    ...
    # silence dis.py bytecode spam (Unsloth monkey-patch side-effect)
    _dis_module.print = lambda *a, **kw: None
    sft_dataset = queries_dataset["train"].map(formatting_prompts_func, batched=True)
    del _dis_module.print
    del queries_dataset
    #######################################
    ...
    del sft_dataset
    if (
        "records_cap" in ctx.sft_training_args
        and ctx.sft_training_args["records_cap"] is not None
        and isinstance(ctx.sft_training_args["records_cap"], int)
    ):
        train_dataset = train_dataset.take(ctx.sft_training_args["records_cap"])
        eval_dataset = eval_dataset.take(ctx.sft_training_args["records_cap"])
    ...
    train_args = UnslothTrainingArguments(
        per_device_train_batch_size=2,
        gradient_accumulation_steps=8,
        **{k: v for k, v in ctx.sft_training_args.items() if k != "records_cap"},
    ...
    used_memory_for_lora = round(used_memory - ctx.start_gpu_memory, 3)
    used_percentage = round(used_memory / ctx.max_memory * 100, 3)
    lora_percentage = round(used_memory_for_lora / ctx.max_memory * 100, 3)
    print("Peak reserved memory = " + f"{used_memory} GB.")
    print("Peak reserved memory for " + f"training = {used_memory_for_lora} " + "GB.")
    print("Peak reserved memory % of " + f"max memory = {used_percentage} %.")
    print(
        "Peak reserved memory for SFT training "
    ...
    ctx.sft_log_history_fig = plot_log_history(
        ctx.sft_log_history, title="Supervised finetuning loss"
    )
    del trainer
...
def evaluate_model(_) -> None:
    ...
    validation_results = infer_validation(
        ...
        batch_size=32,  # 64,
        queries_attr_name=ctx.hf_dataset["attributes"]["query_attr"],
        answers_attr_name=ctx.hf_dataset["attributes"]["answers_attr"],
    ...
    records = eval_df.with_columns(
        (pl.col("answer") == pl.col("completion")).alias("is_ground_truth_identical")
    ).collect()  # engine=ctx.engine)
    print(
    ...
    eval_metrics_df = compute_counts_n_metrics(eval_df, is_format_fault_tolerant=True)
    del eval_df
    ...
    ctx.validation_completions_fig = plot_validation_completions(
...
def model_version_blessing(_) -> None:
    ...
    current_blessed_version_dict = current_blessed_model_version_dict(
        repo_id=ctx.model_repo_id, hf_token=os.getenv("HF_TOKEN", None)
    )
    ...
    if current_blessed_version_dict is None:
        ...
        current_blessed_exec_id = current_blessed_version_dict["exec_id"]
        ...
        ctx.model_version_blessed = (
            ctx.perf_metrics[main_perf_metric_name] >= current_blessed_metric_value
        )
        ...
        if not ctx.model_version_blessed:
            ...
            for execution in ExecutionsIterator(
                ...
                if str(execution.id) == current_blessed_exec_id:
                    ...
                    execution_has_custom_card_artifact = (
                        len(
                            [
                                t
                                for t in execution.get_tasks_with_name(
            ...
            if not ctx.current_blessed_exec:
                ...
                logger.debug(f"ctx.current_blessed_exec : {ctx.current_blessed_exec}")
...
def model_to_hub(_) -> None:
    ...
    # when specifying custom
    # 'pipeline_card_artifacts_path'
    if "model_readme_template.md" in os.listdir(ctx.pipeline_card_artifacts_path):
        template_dir = ctx.pipeline_card_artifacts_path
    else:
        template_dir = os.path.dirname(
            importlib.util.find_spec(
    ...
    print(f"template_dir : '{template_dir}'")
    #############################
    if "model_readme.py" in os.listdir(ctx.pipeline_card_artifacts_path):
    ...
    print(
        ...
        + ("(blessed). " if ctx.model_version_blessed else "(not blessed). ")
        + "May take a while..",
        flush=True,
    )
    ...
    print(
        f"https://huggingface.co/{ctx.model_repo_id}"
...
def infra_validator(_) -> None:
    ...
    if ctx.model_version_blessed:
        ...
        if env_has_docker():
            ...
            ctx.local_serve_is_ready = LocalServeReadinessEnum.FAILURE_NO_DOCKER
...
def pipeline_card(_, task_id: int) -> None:
    ...
    executions_count = ExecutionsIterator(exec_name=ctx.pipeline_name).length()
    ...
    params = {
        ...
        "main_dataset_commit_datetime": ctx.hf_dataset_dict["commit_datetime"],
...
def pipeline_to_hub(_) -> None:
    ...
    subfolder_name = "v" + ctx.model_commit_dict["version_label"] + "_" + timestamp_str
    commit_datetime = datetime.utcnow()
    ...
    if (
        ...
        for candidate_source_file in candidate_source_files:
            file_fullpath = os.path.join(
                ctx.pipeline_card_artifacts_path, candidate_source_file
            )
    ...
    source_code_commit_hash = push_files_to_hub_repo_branch(
        ...
        repo_type="model",
        hf_token=os.environ["HF_TOKEN"], # not None
    )
...
def retrain_pipeline():
    ...
    hf_base_model = DagParam(
        ...
        default={"repo_id": "unsloth/Qwen2.5-1.5B", "commit_hash": ""},
    )

    cpt_training_args = DagParam(
        description="dict with `TrainingArguments` params " + "for the CPT job.",
        default={"warmup_ratio": 0.1, "num_train_epochs": 1},
    )

    sft_training_args = DagParam(
        description="dict with `TrainingArguments` params " + "for the SFT job.",
        default={"warmup_ratio": 0.1, "num_train_epochs": 1},
    )
