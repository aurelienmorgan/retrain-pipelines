...
class LightGbmHpCvWandbFlow(FlowSpec):
    ...
    dask_partitions = Parameter(
        "dask_partitions",
        help="(int) how many Dask partitions shall be used " + "per training_job.",
        default=6,
    )
    ...
    def cross_validation(self):
        ...
        # depends on n_splits
        kf = KFold(n_splits=self.cv_folds, shuffle=True, random_state=1121218)
    ...
    def training_job(self):
        ...
        def _parent_cv_task_info(
            training_job_task: LightGbmHpCvWandbFlow, current_: Current
        ) -> (int, dict):
            """
        ...
        print(
            f"Fold finished with rmse: {self.fold_rmse:.5f}.\n"
        ...
        if "disabled" != self.wandb_run_mode:
            ...
            hp_table = wandb.Table(
                data=[list(self.fold_hp_params.values()) + [self.fold_rmse]],
                columns=list(self.fold_hp_params.keys()) + ["rmse"],
            )
    ...
    def best_hp(self, inputs):
        ...
        self.merge_artifacts(
            inputs, exclude=["cross_validation_id", "hp_rmse", "hp_dict"]
        )
        ...
        self.hp_perfs_list = [
            {
                ...
                **{**input.hp_dict, "rmse": input.hp_rmse},
            }
            for input in inputs
        ]
        print(
            pd.DataFrame(self.hp_perfs_list)
        ...
        print(
            "best_cv_task_id : " + f"{inputs[best_cv_agg_task_idx].cross_validation_id}"
        )
    ...
    def train_model(self):
        ...
        ax.grid(
            axis="x", linestyle="--", color="lightgrey", zorder=2
        )  # vertical gridlines
    ...
    def evaluate_model(self):
        ...
        if "feature_names_in_" in self.encoder.__dict__:
            categ_features_arr = self.encoder.__dict__["feature_names_in_"]
            first_categorical_feature = categ_features_arr[0]
            first_categ_feature__labels = self.encoder.__dict__["categories_"][0]
            ...
            for feature_categ in self.X_test[first_categorical_feature].unique():
                ...
                first_categ_feature__label = first_categ_feature__labels[
                    int(feature_categ)
                ]
    ...
    def model_version_blessing(self):
        ...
        for run in Flow(self.__class__.__name__):
            if (
                ...
                current_blessed_rmse = self.current_blessed_run.data.metrics["rmse"]
                self.model_version_blessed = (
                    self.metrics["rmse"] <= current_blessed_rmse
                )
                print(
                    "new : "
                    + str(self.metrics["rmse"])
                    + " - previous best : "
    ...
    def infra_validator(self):
        ...
        if self.model_version_blessed:
            ...
            model_file = os.path.join(self.serving_artifacts_local_folder, "model.txt")
            ...
            shutil.copy(
                ...
                os.path.join(
                    self.serving_artifacts_local_folder, "Dockerfile.mlserver"
                ),
            )
            ...
            shutil.copy(
                ...
                os.path.join(self.serving_artifacts_local_folder, "settings.json"),
            )
            shutil.copy(
                os.path.join(preprocess_module_dir, "model-settings.json"),
                os.path.join(
                    self.serving_artifacts_local_folder, "model-settings.json"
                ),
            )
            ...
            if not serving_container:
                print("failed spinning the MLServer container", file=sys.stderr)
                ...
                self.local_serve_is_ready = int(endpoint_is_ready("lightgbm-model"))
            elapsed_time = time.time() - start_time
    ...
    def pipeline_card(self):
        ...
        self.metadata = {
            ...
            "license": "MIT License",
            "model_details": {"library": "LightGBM", "version": lgb.__version__},
            "metrics": self.metrics,
            "data_sources": [{"name": "Training Data", "path": self.data_file}],
            "preprocessing": [
                {"name": "Normalization", "description": "Standard scaling of features"}
            ],
            "references": [
                {
        ...
        current.card["default"].append(Table(rows, headers=list(self.metrics.keys())))
        ########################
