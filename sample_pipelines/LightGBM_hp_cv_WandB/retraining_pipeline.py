...
def cross_validation(
    ...
    # depends on n_splits
    kf = KFold(n_splits=ctx.cv_folds, shuffle=True, random_state=1121218)
...
def training_job(payload: TaskPayload, rank: List[int], task_id: int) -> None:
    ...
    logger.info(
        f"cross_validation : {parent_cv_task_info['task_id']}" + f" - {fold_hp_params}"
    )
    ...
    fold_lgb_reg, fold_history, fold_workers_history = dask_regressor_fit(
        X_train,
        y_train,
        X_val,
        y_val,
    ...
    logger.info(
        f"Fold finished with rmse: {fold_rmse:.5f}.\n"
        + "(on last distributed epoch :"
    ...
    if "disabled" != ctx.wandb_run_mode:
        ...
        hp_table = wandb.Table(
            data=[list(fold_hp_params.values()) + [fold_rmse]],
            columns=list(fold_hp_params.keys()) + ["rmse"],
        )
...
def cross_validation_agg(payload: TaskPayload, rank: List[int], task_id: int):
    ...
    logger.info(f"hp values {hp_dict} lead to an rmse of {hp_rmse}.")
...
def train_model(_, task_id: int):
    ...
    model, history, workers_history = dask_regressor_fit(
        ctx.X_train,
        ctx.y_train,
        ctx.X_test,
        ctx.y_test,
    ...
    # training history
    fig = plt.figure()
    plt.plot(epochs, train_loss, label="Training Loss", color="#4B0082")
    plt.plot(epochs, valid_loss, label="Validation Loss", color="#008080")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    ...
    ax.grid(axis="x", linestyle="--", color="lightgrey", zorder=2)  # vertical gridlines
...
def evaluate_model(_, task_id: int):
    ...
    ctx.metrics = {"rmse": rmse, "mae ": mae, "r2": r2}
    ...
    if "feature_names_in_" in ctx.encoder.__dict__:
        categ_features_arr = ctx.encoder.__dict__["feature_names_in_"]
        first_categorical_feature = categ_features_arr[0]
        first_categ_feature__labels = ctx.encoder.__dict__["categories_"][0]
        ...
        for feature_categ in ctx.X_test[first_categorical_feature].unique():
            ...
            slice_filter = ctx.X_test[first_categorical_feature].values == feature_categ
            ...
            first_categ_feature__label = first_categ_feature__labels[int(feature_categ)]
...
def model_version_blessing(_):
    ...
    for execution in ExecutionsIterator(
        ...
        if execution.get_attr("model_version_blessed"):
            ...
            print(
                "new : "
                + str(ctx.metrics["rmse"])
                + " - previous best : "
                + str(current_blessed_rmse)
...
def infra_validator(_):
    ...
    if ctx.model_version_blessed:
        ...
        if not serving_container:
            ...
            try:
                cleanup_docker(
                    container_name="lgbm_reg_serve", image_name="lgbm_reg_serve:1.0"
                )
            except Exception:
                # fail silently
                pass
        elif not await_server_ready():
            print("MLServer server failed to get ready", file=sys.stderr)
            print_container_log_tail("lgbm_reg_serve", 50)
            ctx.local_serve_is_ready = 0
        elif not server_has_model("lightgbm-model"):
            print("MLServer server didn't publish the model", file=sys.stderr)
            ctx.local_serve_is_ready = 0
        else:
            ...
            while endpoint_still_starting("lightgbm-model"):
                if time.time() - start_time > timeout:
                    print(
                        "The endpoint 'lightgbm-model' did not start "
            ...
            ctx.local_serve_is_ready = int(endpoint_is_ready("lightgbm-model"))
        elapsed_time = time.time() - start_time
        ...
        if ctx.local_serve_is_ready == 1:
            ...
            try:
                ...
                assert endpoint_response and len(endpoint_response) == len(
                    raw_inference_request_items["inputs"][0]["data"]
                ), (
...
def retrain_pipeline():
    ...
    cv_folds = DagParam(
        description="(int) how many Cross Validation folds shall be used.", default=3
    )
