...
from retrain_pipelines.utils import flatten_dict, dict_dict_list_get_all_combinations
...
def preprocess_data(payload: TaskPayload) -> TaskPayload:
    ...
    preprocess_data_fct = get_preprocess_data_fct(ctx.preprocess_artifacts_path)
...
def split_data(payload: TaskPayload) -> None:
    ...
    logger.info(
        f"train set : {len(ctx.X_train):,}, " + f"holdout set : {len(ctx.X_test):,}"
    )
...
def hyper_tuning(_):
    ...
    all_hp_params = dict_dict_list_get_all_combinations(ctx.pipeline_hp_grid)

    for i, hp_params in enumerate(all_hp_params):
        logger.info(f"hp_params[{i}] : " + json.dumps(hp_params, indent=4, default=str))
...
def cross_validation(
    ...
    logger.info("hp_params : " + json.dumps(hp_params, indent=4, default=str))
    logger.info(f"{ctx.cv_folds} cross-validation folds")

    strat_kf = StratifiedKFold(
        n_splits=ctx.cv_folds, shuffle=True, random_state=1121218
    )
    all_fold_splits = list(strat_kf.split(ctx.X_train, ctx.y_train))
    ...
    ctx.cross_validation[str(rank)] = {"task_id": task_id, "hp": hp_params}
    #################################################
...
@parallel_task
def training_job(payload: TaskPayload, rank: List[int], task_id: int) -> None:
        ...
        parallel_run = wandb.init(
            ...
            config={
                ...
                **dict(
                    ...
                    rp_task="_".join(
                        [func_name, str(parent_cv_task_info["task_id"]), str(task_id)]
                    ),
                ),
            },
    ...
    X_train, y_train = ctx.X_train.iloc[train_idx], ctx.y_train.iloc[train_idx]
    X_val, y_val = ctx.X_train.iloc[val_idx], ctx.y_train.iloc[val_idx]

    # Add eval_set and eval_names to track evaluation results
    eval_set = [(ctx.X_train.values, ctx.y_train), (ctx.X_test.values, ctx.y_test)]
    eval_names = ["Training", "Validation"]

    tabnet_clf = TabNetClassifier(
        **fold_hp_params.get("model", {}), grouped_features=ctx.grouped_features
    )
    tabnet_clf.fit(
        ...
        **fold_hp_params.get("trainer", {}),
        ...
        drop_last=False,
        callbacks=(
            [PrintLR()] + [WandbCallback()] if "disabled" != ctx.wandb_run_mode else []
        ),
    )
    ...
    if "disabled" != ctx.wandb_run_mode:
        ...
        cv_input_flattened_dict = flatten_dict(fold_hp_params, callable_to_name=True)
...
def avg_scalars(scalars_list: List[float]):
    return float(np.mean(scalars_list))


@task(merge_func=avg_scalars)
def cross_validation_agg(payload: TaskPayload, rank: List[int], task_id: int):
    """
    ...
    return {
        ...
        "rp:rank": str(rank),
        **{**flatten_dict(hp_dict, callable_to_name=True), "accuracy": hp_accuracy},
    }


def best_cv_perfs_rank(cv_perf_dicts_list: dict):
    """
    Returns the rank of the
    best-performing set of hyperparameters.
    """
    print(
        pd.DataFrame(cv_perf_dicts_list)
    ...
    best_cv_rank = cv_perf_dicts_list[best_cv_agg_task_idx]["rp:rank"]

    ctx.hp_perfs_list = [
        {k: v for k, v in d.items() if k != "rank"} for d in cv_perf_dicts_list
    ]
...
def best_hp(payload: TaskPayload):
    """
    ...
    print("hyperparameters : " + json.dumps(ctx.hyperparameters, indent=4, default=str))
...
def train_model(_, task_id: int):
    ...
    model = TabNetClassifier(
        **ctx.hyperparameters.get("model", {}), grouped_features=ctx.grouped_features
    )
    ...
    if "disabled" != ctx.wandb_run_mode:
        ...
        wandb_code_root = os.path.realpath(os.path.dirname(os.path.abspath(__file__)))
        print(f"wandb log code from : '{wandb_code_root}'")
        training_run.log_code(
            root=wandb_code_root,
            include_fn=lambda path: (path.endswith(".py") or path.endswith(".html")),
        )
...
def evaluate_model(_, task_id: int):
    ...
    precision = precision_score(ctx.y_test, ctx.predictions, average="weighted")
    recall = recall_score(ctx.y_test, ctx.predictions, average="weighted")
    f1 = f1_score(ctx.y_test, ctx.predictions, average="weighted")
    ...
    if "disabled" != ctx.wandb_run_mode:
        ...
        wandb_flow_run.log(
            dict(
                metrics=ctx.classes_weighted_metrics,
                # for charts
                hp_cv_acc=ctx.classes_weighted_metrics["accuracy"],
            )
        )
    ...
    encoder_params_str = read_text_file(
        ctx.serving_artifacts_storage_location,
        ["encoder_params.json"],
    )
    encoder_dict = json.loads(encoder_params_str)
    if encoder_dict:
        ...
        for feature_categ in encoder_dict[first_categorical_feature]:
            feature_column_name = "_".join([first_categorical_feature, feature_categ])
            # Get test-records indices for this category
            slice_filter = ctx.X_test[feature_column_name].values == 1
            ...
            recall = recall_score(sliced_y_test, sliced_predictions, average="weighted")
        ...
        ctx.first_categorical_feature = first_categorical_feature
        ctx.first_categ_feature_sliced_metrics = first_categ_feature_sliced_metrics
        print(
    ...
    for execution in ExecutionsIterator(
        ...
        if (
            ...
            current_blessed_accuracy = float(
                exec_blessing_context["classes_weighted_metrics"]["accuracy"]
            )
            ...
            print(
                "new : "
                + str(ctx.classes_weighted_metrics["accuracy"])
                + " - previous best : "
...
def infra_validator(_):
    ...
    if ctx.model_version_blessed:
        ...
        if is_s3_path(ctx.serving_artifacts_storage_location):
            # PyTorch save_model expects a local file path.
            # If the target is S3, we save to a temporary local file
            # and then upload it to the target S3 location.
            with tempfile.TemporaryDirectory() as tmp_dir:
                tmp_model_zip_path = os.path.join(tmp_dir, "model")
                ctx.model.save_model(tmp_model_zip_path)
                copy_local_dir_to_s3(tmp_dir, ctx.serving_artifacts_storage_location)
        else:
            model_zip_filefullname = os.path.join(ctx.serving_artifacts_storage_location,
                                                  "model")
            ctx.model.save_model(model_zip_filefullname)
        ...
        write_text_file(
            ...
            read_text_file(preprocess_module_dir, ["Dockerfile.torchserve"]),
        )
        # save TabNet TorchServe handler class as artifact
        write_text_file(
            ctx.serving_artifacts_storage_location,
            ["torchserve_tabnet_handler.py"],
            read_text_file(preprocess_module_dir, ["torchserve_tabnet_handler.py"]),
        )
        # save dependencies as artifact
        write_text_file(
            ctx.serving_artifacts_storage_location,
            ["requirements.txt"],
            "", # (just create an empty one here)
        )
        # create_requirements(ctx.serving_artifacts_storage_location,
        # exclude=[
        ...
        serving_container = build_and_run_docker(
            ...
            ports_publish_dict={"8080/tcp": 9080, "8081/tcp": 9081, "8082/tcp": 9082},
        )
        if not serving_container:
            ...
            timeout = 10 * 60
            print(f"Awaiting endpoint launch [timeout {timeout} seconds]..")
            start_time = time.time()
            ...
            ctx.local_serve_is_ready = int(endpoint_is_ready("tabnet_model"))
        elapsed_time = time.time() - start_time
        ...
        if ctx.local_serve_is_ready == 1:
            ...
            try:
                ...
                ctx.local_serve_is_ready = 1
                elapsed_time = time.time() - start_time
                print(f"inference test -   Elapsed time: {elapsed_time:.2f} seconds")
                ...
                pass
        try:
            cleanup_docker(container_name="tabnet_serve", image_name="tabnet_serve:1.0")
        except Exception:
            # fail silently
            pass
...
def pipeline_card(_, task_id: int):
    ...
    params = {
        ...
        "sliced_perf_metrics_dict": ctx.first_categ_feature_sliced_metrics,
        "buckets_dict": ctx.buckets_param,
        "hyperparameters_dict": flatten_dict(
            ctx.hyperparameters, callable_to_name=True
        ),
        "wandb_project_ui_url": (
            ctx.wandb_project_ui_url if "disabled" != ctx.wandb_run_mode else None
        ),
        "wandb_filter_run_id": (
            ctx.wandb_filter_run_id if "disabled" != ctx.wandb_run_mode else None
        ),
        "wandb_need_sync_dir": (
            ""
            if "offline" != ctx.wandb_run_mode
            else ctx.wandb_run_dir + os.path.sep.join(["", "wandb", "offline-*"])
        ),
        "pipeline_hp_grid_dict": flatten_dict(
            ctx.pipeline_hp_grid, callable_to_name=True
        ),
...
def retrain_pipeline():
    ...
    pipeline_hp_grid = DagParam(
        ...
        default={
            ...
            "model": {
                ...
                "scheduler_params": [{"gamma": 0.80, "step_size": 20}],
    ...
    cv_folds = DagParam(
        description="(int) how many Cross Validation folds shall be used.", default=2
    )
