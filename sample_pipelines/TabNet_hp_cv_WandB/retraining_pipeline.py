...
def evaluate_model(_, task_id: int):
    ...
    encoder_file = os.path.join(
        ctx.serving_artifacts_local_folder, "encoder_params.json"
    )
...
def infra_validator(_):
    ...
    if ctx.model_version_blessed:
        ...
        model_file = os.path.join(ctx.serving_artifacts_local_folder, "model")
        ...
        shutil.copy(
            ...
            os.path.join(ctx.serving_artifacts_local_folder, "Dockerfile.torchserve"),
        )
        # save TabNet TorchServe handler class as artifact
        shutil.copy(
            os.path.join(preprocess_module_dir, "torchserve_tabnet_handler.py"),
            os.path.join(
                ctx.serving_artifacts_local_folder, "torchserve_tabnet_handler.py"
            ),
        )
        # save dependencies as artifact
        open(
            f"{ctx.serving_artifacts_local_folder}/requirements.txt", "w"
        ).close()  # just create an empty one here
