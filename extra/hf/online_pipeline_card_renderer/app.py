...
from huggingface_hub import list_repo_files, list_repo_commits, hf_hub_download
...
def handle_exception(error):
    ...
    app.logger.error(
        f"{request.remote_addr} - Exception occurred: {error_msg}", exc_info=error
    )

    return render_template(ERROR_PAGE, error_msg=error_msg), status_code
...
def preview_html():
    ...
    if not model_repo_id:
        ...
        app.logger.error(f"{request.remote_addr} - {error_msg}", exc_info=False)
        return render_template(ERROR_PAGE, error_msg=error_msg), 400
    ...
    try:
        ...
        app.logger.error(f"{request.remote_addr} - {error_msg}", exc_info=False)
        return render_template(ERROR_PAGE, error_msg=error_msg), 500


def get_pipeline_card_filename(
    model_repo_id: str, banch_name: str, subfolder: str, hf_token: str
) -> str:
    """
    ...
    files = list_repo_files(
        repo_id=model_repo_id, repo_type="model", revision=banch_name, token=hf_token
    )
